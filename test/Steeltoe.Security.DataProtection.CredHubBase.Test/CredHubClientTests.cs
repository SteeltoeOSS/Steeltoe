...
namespace Steeltoe.Security.DataProtection.CredHub.Test
{
    public class CredHubClientTests
    {
        ...
        [Fact]
        public async void Create_WithClientCert_ExpectsEnvVars()
        {
            // Validate key validation
            Environment.SetEnvironmentVariable("CF_INSTANCE_CERT", "asdf");

            // await Assert.ThrowsAsync<ArgumentException>(() => CredHubClient.CreateAsync(string.Empty));
            await Assert.ThrowsAsync<ArgumentException>(() => CredHubClient.CreateMTLSClientAsync(new CredHubOptions()));

            // Validate cert validation
            Environment.SetEnvironmentVariable("CF_INSTANCE_CERT", string.Empty);
            Environment.SetEnvironmentVariable("CF_INSTANCE_KEY", "asdf");

            // await Assert.ThrowsAsync<ArgumentException>(() => CredHubClient.CreateAsync(string.Empty));
            await Assert.ThrowsAsync<ArgumentException>(() => CredHubClient.CreateMTLSClientAsync(new CredHubOptions()));
        }

        ////[Fact(Skip = "Requires local CredHub server, with certs generated by it")]
        ////public async void ClientCertificates_Work()
        ////{
        ////    // arrange
        ////    Environment.SetEnvironmentVariable("CF_INSTANCE_CERT", "c:/temp/client.pem");
        ////    Environment.SetEnvironmentVariable("CF_INSTANCE_KEY", "c:/temp/client_key.pem");

        ////    // act
        ////    var client = await CredHubClient.CreateMTLSClientAsync(new CredHubOptions { CredHubUrl = "https://localhost:9000", ValidateCertificates = false });
        ...
        public async void WriteAsyncUserWithPermissions_Sets_UserWithPermissions()
        {
            ...
            var mockRequest = mockHttpMessageHandler
                .Expect(HttpMethod.Put, credHubBase + "/v1/data")
                .WithContent("{\"value\":{\"username\":\"testUser\",\"password\":\"testPassword\"},\"overwrite\":false,\"additional_permissions\":[{\"actor\":\"test\",\"operations\":[\"read\",\"write\"]}],\"name\":\"example-user\",\"type\":\"User\"}")
        ...
        public async void GenerateAsync_Creates_Password()
        {
            ...
            var mockRequest = mockHttpMessageHandler
                ...
                .WithContent("{\"overwrite\":false,\"parameters\":{\"length\":40},\"name\":\"generated-password\",\"type\":\"Password\"}")
        ...
        public async void GenerateAsync_Creates_User()
        {
            ...
            var mockRequest = mockHttpMessageHandler
                .Expect(HttpMethod.Post, credHubBase + "/v1/data")
                .WithContent("{\"overwrite\":false,\"parameters\":{\"length\":40},\"name\":\"generated-user\",\"type\":\"User\"}")
        ...
        public async void GenerateAsync_Creates_Certificate()
        {
            ...
            var mockRequest = mockHttpMessageHandler
                .Expect(HttpMethod.Post, credHubBase + "/v1/data")
                .WithContent("{\"overwrite\":false,\"parameters\":{\"common_name\":\"TestCA\",\"duration\":365,\"is_ca\":true,\"self_sign\":false,\"key_length\":2048},\"name\":\"example-ca\",\"type\":\"Certificate\"}")
        ...
        public async void GenerateAsync_Creates_RSA()
        {
            ...
            var mockRequest = mockHttpMessageHandler
                .Expect(HttpMethod.Post, credHubBase + "/v1/data")
                .WithContent("{\"overwrite\":false,\"parameters\":{\"key_length\":2048},\"name\":\"example-rsa\",\"type\":\"RSA\"}")
                .Respond("application/json", "{\"type\":\"rsa\",\"version_created_at\":\"2017-11-10T15:55:24Z\",\"id\":\"2af5191f-9c05-4746-b72c-78b3283aef46\",\"name\":\"/example-rsa\",\"value\":{\"public_key\":\"-----BEGIN PUBLIC KEY-----\\nFakePublicKeyTextEAAQ==\\n-----END PUBLIC KEY-----\\n\",\"private_key\":\"-----BEGIN RSA PRIVATE KEY-----\\nFakePrivateKeyTextEAAQ==\\n-----END RSA PRIVATE KEY-----\\n\"}}");
            var client = await InitializeClientAsync(mockHttpMessageHandler);
        ...
        public async void GenerateAsync_Creates_SSH()
        {
            ...
            var mockRequest = mockHttpMessageHandler
                .Expect(HttpMethod.Post, credHubBase + "/v1/data")
                .WithContent("{\"overwrite\":false,\"parameters\":{\"key_length\":2048},\"name\":\"example-ssh\",\"type\":\"SSH\"}")
            ...
            var client = await InitializeClientAsync(mockHttpMessageHandler);
            var request = new SshGenerationRequest("example-ssh", CertificateKeyLength.Length_2048);
        ...
        {
            return CredHubClient.CreateUAAClientAsync(new CredHubOptions { CredHubUrl = credHubBase, CredHubUser = "credHubUser", CredHubPassword = "credHubPassword" }, httpClient: new HttpClient(mockHttpMessageHandler));
        }
