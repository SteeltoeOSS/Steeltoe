...
public class HystrixRequestEventStreamControllerTest : HystrixTestBase
{
    ...
    public async Task Endpoint_ReturnsHeaders()
    {
        ...
        HttpResponseMessage result = await client.SendAsync(
            new HttpRequestMessage(HttpMethod.Get, "hystrix/request.stream"), HttpCompletionOption.ResponseHeadersRead);
    ...
    public void Endpoint_ReturnsData()
    {
        ...
        client.BaseAddress = new Uri("http://localhost/");
        Stream result = client.GetStreamAsync("hystrix/request.stream").GetAwaiter().GetResult();
