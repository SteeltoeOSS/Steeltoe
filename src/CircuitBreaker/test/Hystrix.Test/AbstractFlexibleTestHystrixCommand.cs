...
internal abstract class AbstractFlexibleTestHystrixCommand : TestHystrixCommand<int>
{
    ...
    protected AbstractFlexibleTestHystrixCommand(IHystrixCommandKey commandKey, ExecutionIsolationStrategy isolationStrategy,
        ...
        : base(TestPropsBuilder(circuitBreaker).SetCommandKey(commandKey).SetCircuitBreaker(circuitBreaker).SetMetrics(circuitBreaker.Metrics)
            ...
            .SetExecutionSemaphore(executionSemaphore).SetFallbackSemaphore(fallbackSemaphore))
    {
    ...
    protected override int Run()
    {
        ...
        if (Result == ExecutionResultTest.Failure)
        {
            throw new Exception("Execution Failure for TestHystrixCommand");
        }

        if (Result == ExecutionResultTest.HystrixFailure)
        {
            throw new HystrixRuntimeException(FailureType.CommandException, typeof(AbstractFlexibleTestHystrixCommand),
                "Execution Hystrix Failure for TestHystrixCommand", new Exception("Execution Failure for TestHystrixCommand"),
                new Exception("Fallback Failure for TestHystrixCommand"));
        }

        if (Result == ExecutionResultTest.RecoverableError)
        {
            throw new Exception("Execution ERROR for TestHystrixCommand");
        }

        if (Result == ExecutionResultTest.UnrecoverableError)
        {
            throw new OutOfMemoryException("Unrecoverable Error for TestHystrixCommand");
        }

        if (Result == ExecutionResultTest.BadRequest)
        {
    ...
    protected void AddLatency(int latency)
    {
        if (latency > 0)
        {
            try
            {
                Output?.WriteLine(DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond + " : " + Thread.CurrentThread.ManagedThreadId + " About to sleep for : " +
                    latency);
            ...
            catch (Exception e)
            {
                ...
                Output?.WriteLine(DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond + " : " + Thread.CurrentThread.ManagedThreadId +
                    "after interruption with extra sleep");
    ...
    private static HystrixCommandOptions GetTestOptions(HystrixCommandOptions hystrixCommandOptions, ExecutionIsolationStrategy isolationStrategy, int timeout,
