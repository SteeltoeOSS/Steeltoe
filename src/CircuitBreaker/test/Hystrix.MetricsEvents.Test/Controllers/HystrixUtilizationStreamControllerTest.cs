...
public class HystrixUtilizationStreamControllerTest : HystrixTestBase
{
    ...
    public async Task Endpoint_ReturnsHeaders()
    {
        ...
        var requestUri = new Uri("hystrix/utilization.stream", UriKind.Relative);
    ...
    public async Task Endpoint_ReturnsData()
    {
        ...
        client.BaseAddress = new Uri("http://localhost/");
        Stream result = await client.GetStreamAsync(new Uri("hystrix/utilization.stream", UriKind.Relative));
