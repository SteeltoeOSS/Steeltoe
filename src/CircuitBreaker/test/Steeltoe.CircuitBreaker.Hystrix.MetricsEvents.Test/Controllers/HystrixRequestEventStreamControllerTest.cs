    ...
    public class HystrixRequestEventStreamControllerTest : HystrixTestBase
    {
        ...
        public async void Endpoint_ReturnsHeaders()
        {
            ...
            using (var server = new TestServer(builder))
            {
                ...
                var result = await client.SendAsync(new HttpRequestMessage(HttpMethod.Get, "hystrix/request.stream"),
                ...
                Assert.True(result.Headers.Connection.Contains("keep-alive"));
        ...
        public void Endpoint_ReturnsData()
        {
            ...
            using (var server = new TestServer(builder))
            {
                ...
                client.BaseAddress = new Uri("http://localhost/");
                var result = client.GetStreamAsync("hystrix/request.stream").Result;
                ...
                Assert.True(data.StartsWith("data: "));
