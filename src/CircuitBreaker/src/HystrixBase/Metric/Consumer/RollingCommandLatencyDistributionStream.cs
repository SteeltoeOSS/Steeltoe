...
{
    private static readonly ConcurrentDictionary<string, RollingCommandLatencyDistributionStream> Streams = new();
    ...
    {
        RollingCommandLatencyDistributionStream result = Streams.GetOrAddEx(commandKey.Name, _ =>
        {
    ...
    {
        foreach (RollingCommandLatencyDistributionStream stream in Streams.Values)
        {
            stream.Unsubscribe();
        }
