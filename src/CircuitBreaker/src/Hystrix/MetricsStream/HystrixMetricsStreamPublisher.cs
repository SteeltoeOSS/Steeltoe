...
public class HystrixMetricsStreamPublisher : IDisposable
{
    ...
    public HystrixMetricsStreamPublisher(IOptions<HystrixMetricsStreamOptions> options, HystrixDashboardStream stream, ILogger logger = null,
    ...
    public void StartMetricsPublishing()
    {
        ...
        SampleSubscription = observable.ObserveOn(NewThreadScheduler.Default).Subscribe(jsonList =>
        {
            try
            {
                if (EnsureConnection())
