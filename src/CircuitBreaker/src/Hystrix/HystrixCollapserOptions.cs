...
public class HystrixCollapserOptions : HystrixBaseOptions, IHystrixCollapserOptions
{
    ...
    public HystrixCollapserOptions(IHystrixCollapserKey key, RequestCollapserScope scope, IHystrixCollapserOptions defaults = null,
        ...
        : base(dynamic)
    {
        ...
        TimerDelayInMilliseconds = GetInteger(HystrixCollapserPrefix, key.Name, "timerDelayInMilliseconds", DefaultTimerDelayInMilliseconds,
            defaults?.TimerDelayInMilliseconds);
        ...
        MetricsRollingStatisticalWindowInMilliseconds = GetInteger(HystrixCollapserPrefix, key.Name, "metrics.rollingStats.timeInMilliseconds",
            DefaultMetricsRollingStatisticalWindow, defaults?.MetricsRollingStatisticalWindowInMilliseconds);

        MetricsRollingStatisticalWindowBuckets = GetInteger(HystrixCollapserPrefix, key.Name, "metrics.rollingStats.numBuckets",
            DefaultMetricsRollingStatisticalWindowBuckets, defaults?.MetricsRollingStatisticalWindowBuckets);

        MetricsRollingPercentileEnabled = GetBoolean(HystrixCollapserPrefix, key.Name, "metrics.rollingPercentile.enabled",
            DefaultMetricsRollingPercentileEnabled, defaults?.MetricsRollingPercentileEnabled);

        MetricsRollingPercentileWindowInMilliseconds = GetInteger(HystrixCollapserPrefix, key.Name, "metrics.rollingPercentile.timeInMilliseconds",
            DefaultMetricsRollingPercentileWindow, defaults?.MetricsRollingPercentileWindowInMilliseconds);

        MetricsRollingPercentileWindowBuckets = GetInteger(HystrixCollapserPrefix, key.Name, "metrics.rollingPercentile.numBuckets",
            DefaultMetricsRollingPercentileWindowBuckets, defaults?.MetricsRollingPercentileWindowBuckets);

        MetricsRollingPercentileBucketSize = GetInteger(HystrixCollapserPrefix, key.Name, "metrics.rollingPercentile.bucketSize",
