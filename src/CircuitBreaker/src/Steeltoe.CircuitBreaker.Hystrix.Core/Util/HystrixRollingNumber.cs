...
namespace Steeltoe.CircuitBreaker.Hystrix.Util
{
    public class HystrixRollingNumber
    {
        ...
        internal readonly CumulativeSum cumulativeSum = new CumulativeSum();
        ...
        internal class Bucket
        {
            ...
            public readonly LongAdder[] adderForCounterType;
            public readonly LongMaxUpdater[] updaterForCounterType;
       ...
       internal class CumulativeSum
        {
            readonly LongAdder[] adderForCounterType;
            readonly LongMaxUpdater[] updaterForCounterType;
        ...
        internal class BucketCircularArray : IEnumerable<Bucket>
        {
            ...
            class ListState
            {
                ...
                public readonly BucketCircularArray ca;
                ...
                public ListState(BucketCircularArray ca, AtomicReferenceArray<Bucket> data, int head, int tail)
                {
                    ...
                    {
                        this.size = (tail + ca.dataLength - head) % ca.dataLength;
                    }
                    this.data = data;
                }
                ...
                private ListState IncrementTail()
                {
                    ...
                    if (size == ca.numBuckets)
                    {
                        // increment tail and head
                        return new ListState(ca, data, (head + 1) % ca.dataLength, (tail + 1) % ca.dataLength);
                    }
                    else
                    {
                        // increment only tail
                        return new ListState(ca, data, head, (tail + 1) % ca.dataLength);
                    }
                }

                public ListState Clear()
                {
                    return new ListState(ca, new AtomicReferenceArray<Bucket>(ca.dataLength), 0, 0);
                }
                ...
                {
                    return (index + head) % ca.dataLength;
                }
            }
