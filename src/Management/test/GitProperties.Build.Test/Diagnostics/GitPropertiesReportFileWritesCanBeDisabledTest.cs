...
public sealed class GitPropertiesReportFileWritesCanBeDisabledTest : GitPropertiesBuildTestBase
{
    ...
    public async Task Test()
    {
        ...
        DotNetCommandOutput disabledOutput =
            await repository.TestApp.BuildAsync("-p:GitPropertiesWriteToProjectDirectory=true", "-p:GitPropertiesReportFileWrites=false");

        disabledOutput.Value.Should().NotContain("git.properties: generating shared cache");
        disabledOutput.Value.Should().NotContain("git.properties: writing to");
        disabledOutput.Value.Should().NotContain("git.properties: writing fallback copy to");
