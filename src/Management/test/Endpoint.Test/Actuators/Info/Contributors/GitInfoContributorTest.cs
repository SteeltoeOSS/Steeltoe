...
{
    [Fact]
    public void Default_path_prefers_assembly_directory_over_current_directory()
    {
        ...
        string outputPath = GitInfoContributor.ResolveDefaultPropertiesPath(baseDirectory.FullPath, currentDirectory.FullPath);

        outputPath.Should().Be(baseDirectoryFile);
    }

    [Fact]
    public void Default_path_falls_back_to_current_directory_when_not_found_in_assembly_directory()
    {
        using var baseDirectory = new Sandbox();
        using var currentDirectory = new Sandbox();
        ...
        outputPath.Should().Be(currentDirectoryFile);
    }

    [Fact]
    public void Default_path_uses_current_directory_when_not_found_anywhere()
    {
        using var baseDirectory = new Sandbox();
        using var currentDirectory = new Sandbox();
        ...
        outputPath.Should().Be(Path.Combine(currentDirectory.FullPath, "git.properties"));
    }
