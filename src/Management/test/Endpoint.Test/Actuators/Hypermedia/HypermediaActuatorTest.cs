...
public sealed class HypermediaActuatorTest
{
    ...
    [Fact]
    public async Task Can_use_PathBase()
    {
        var appSettings = new Dictionary<string, string?>
        {
            //["Management:Endpoints:Path"] = "/prefix/actuator"
        };
        ...
        HttpResponseMessage response = await httpClient.GetAsync(new Uri("http://localhost/prefix/actuator"), TestContext.Current.CancellationToken);
        ...
        responseBody.Should().BeJson("""
            {
              ...
              "_links": {
                "info": {
                  "href": "http://localhost/prefix/actuator/info",
                  "templated": false
                },
                "self": {
                  "href": "http://localhost/prefix/actuator",
                  "templated": false
                }
              }
            }
