...
namespace OpenCensus.Trace.Export
{
    ...
    public sealed class InProcessSampledSpanStore : SampledSpanStoreBase
    {
        ...
        public override void ConsiderForSampling(ISpan ispan)
        {
            if (ispan is SpanBase span)
            {
                lock (samples)
                {
                    ...
                    samples.TryGetValue(spanName, out var perSpanNameSamples);
        ...
        public override IEnumerable<ISpanData> GetErrorSampledSpans(ISampledSpanStoreErrorFilter filter)
        {
            ...
            lock (samples)
            {
                var perSpanNameSamples = samples[filter.SpanName];
                if (perSpanNameSamples != null)
                {
            ...
            var ret = new List<ISpanData>(spans.Count);
        ...
        private sealed class Bucket
        {
                ...
                EvictingQueue<SpanBase> queue)
            {
                ...
                foreach (var span in copy)
                {
                    ...
                    var spanLatencyNs = span.LatencyNs;
            ...
            {
                var spanEndNanoTime = span.EndNanoTime;
                if (span.Context.TraceOptions.IsSampled)
                {
                    ...
                    if (spanEndNanoTime - lastSampledNanoTime > TimeBetweenSamples)
                    {
                ...
                else
                {
                    ...
                    // this may never sample again (at least for the next ~200 years). No real chance to
                    // overflow two times because that means the process runs for ~200 years.
                    if (spanEndNanoTime - lastNotSampledNanoTime > TimeBetweenSamples)
                    {
        ...
        private sealed class PerSpanNameSamples
        {
            ...
            {
                latencyBuckets = new Bucket[NumLatencyBuckets];
                for (var i = 0; i < NumLatencyBuckets; i++)
                {
                    latencyBuckets[i] = new Bucket(NumSampolesPerLatencySamples);
                }

                errorBuckets = new Bucket[NumErrorBuckets];
                for (var i = 0; i < NumErrorBuckets; i++)
                {
                    errorBuckets[i] = new Bucket(NumSamplesPerErrorSamples);
                }
            }
