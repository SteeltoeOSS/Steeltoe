...
namespace Steeltoe.Management.OpenTelemetry.Metrics
{
    ...
    {
        private readonly DoubleMeasureMinMaxSumCountAggregator _measureAggregator = new ();

        public override void Record(in SpanContext context, double value) => _measureAggregator.Update(value);

        public override Aggregator<double> GetAggregator() => _measureAggregator;
    }
}
