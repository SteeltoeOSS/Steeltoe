...
internal sealed class PullMetricsCollectionManager
{
    ...
    public Task<ICollectionResponse> EnterCollectAsync()
    {
        ...
        if (_previousDataViewGeneratedAtUtc.HasValue && _scrapeResponseCacheDurationInMilliseconds > 0 &&
        ...
        {
            _collectionTcs ??= new TaskCompletionSource<ICollectionResponse>(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    private bool ExecuteCollect()
    {
        ...
        bool? result = _exporter.Collect?.Invoke(Timeout.Infinite);
