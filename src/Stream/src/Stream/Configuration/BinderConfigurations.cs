...
public class BinderConfigurations : IBinderConfigurations
{
    ...
    public BinderConfigurations(IBinderTypeRegistry binderTypeRegistry, IConfiguration configuration)
    {
        Registry = binderTypeRegistry;
        Options = new BindingServiceOptions(configuration);
    }
    ...
    public List<string> FindMatchingConfigurationsIfAny(IBinder binder)
    {
        ...
        {
            foreach (KeyValuePair<string, BinderConfiguration> configuration in Configurations)
            {
                if (registryEntry.AssemblyPath == configuration.Value.ResolvedAssembly)
                {
                    results.Add(configuration.Key);
                }
            }
        }
