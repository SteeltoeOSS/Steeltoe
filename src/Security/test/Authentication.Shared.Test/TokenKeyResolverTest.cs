  ...
  ""keys"": [
    {
      ...
      ""e"": ""AQAB""
        ...
        TokenKeyResolver.Resolved.Clear();

        var resolver = new TokenKeyResolver("https://foo.bar", new HttpClient());
        TokenKeyResolver.Resolved["legacy-token-key"] = webKey;
  ...
  ""keys"": [
    {
      ...
      ""e"": ""AQAB""
        ...
        Assert.NotNull(TokenKeyResolver.Resolved["legacy-token-key"]);
  ...
  ""keys"": [
    {
      ...
      ""e"": ""AQAB""
        ...
        IEnumerable<SecurityKey> result = resolver.ResolveSigningKey(token, null!, "legacy-token-key", null!);
        Assert.NotNull(handler.LastRequest);
        Assert.False(TokenKeyResolver.Resolved.ContainsKey("legacy-token-key"));
        Assert.Empty(result);
    }
