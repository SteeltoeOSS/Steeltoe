...
namespace Steeltoe.Management.Diagnostics.Listeners
{
    public abstract class DiagnosticObserver : IDiagnosticObserver
    {
        ...
        protected DiagnosticObserver(string listenerName, ILogger logger = null)
        {
        ...
        public void Dispose()
        {
            ...
            Subscription = null;
            Logger?.LogInformation("DiagnosticObserver {observer} Disposed", ListenerName);
        }

        public void Subscribe(DiagnosticListener listener)
        {
            if (ListenerName.StartsWith(listener.Name))
            {
                if (Subscription != null)
                {
                    Dispose();
                ...
                Logger?.LogInformation("DiagnosticObserver {observer} Subscribed to {listener}", ListenerName, listener.Name);
            }
        }
        ...
        public abstract void ProcessEvent(string key, object value);
    }
}
