...
internal sealed class DynamicPortAssignmentHostedService : IHostedLifecycleService
{
    ...
    internal sealed class EurekaInstanceDynamicPortsPostConfigureOptions : IPostConfigureOptions<EurekaInstanceOptions>
    {
        ...
        public EurekaInstanceDynamicPortsPostConfigureOptions(AspNetServerListenState listenState)
        {
            ArgumentGuard.NotNull(listenState);
        ...
        {
            if (_listenState.ListenOnAddresses is { Count: > 0 } && string.IsNullOrEmpty(options.RegistrationMethod))
            {
                options.IsNonSecurePortEnabled = false;
                options.IsSecurePortEnabled = false;

                foreach (string address in _listenState.ListenOnAddresses)
                {
                    if (Uri.TryCreate(address, UriKind.Absolute, out Uri? uri))
                    {
                        if (uri is { Scheme: "http", Port: > 0 } && !options.IsNonSecurePortEnabled)
                        {
                            options.NonSecurePort = uri.Port;
                            options.IsNonSecurePortEnabled = true;
                        }
                        else if (uri is { Scheme: "https", Port: > 0 } && !options.IsSecurePortEnabled)
                        {
